...
//the statement inside the do segment will be repeated until the number entered is positive
    ...
    if (pid>0) {
        ...
        if (shm_base == MAP_FAILED) {
            printf("Error. Mapping the shared memory segment ot the adress space of the parent failed.">            exit(1);
        }
        pid=wait(&status);
        ...
        shm_base = mmap(0, SIZE, PROT_READ | PROT_WRITE, MAP_SHARED,shm_fd, 0);
         if (shm_base == MAP_FAILED) {
            printf("Error. Mapping the shared memory segment ot the adress space of the child failed.");            exit(1);
                   }
